! function(a) {
    ...
    function b(b, c) {
        a("#messages").empty(), a("#messages").append('<div class="alert alert-' + c + '" role="alert">' + b.message + "</div>")
    }
    a("#contactForm").on("submit", function(c) {
        var d = a(this),
            e = d.find("button");
        return c.preventDefault(), d.ajaxSubmit({
            url: "contact_mailer.php",
            type: "post",
            dataType: "json",
            beforeSubmit: function() {
                e.attr("disabled", "disabled"), e.find("i").removeClass().addClass("fa fa-circle-o-notch fa-spin")
            },
            success: function(a, c, d, f) {
                if ("ok" === a.status) {
                    for (var g in a.messages) b(a.messages[g], "success");
                    f[0].reset()
                } else
                    for (var h in a.messages) b(a.messages[h], "danger");
                e.removeAttr("disabled"), e.find("i").removeClass().addClass("fa fa-envelope")
            },
            error: function(a) {
                for (var c in a.messages) b(a.messages[c], "danger");
                e.removeAttr("disabled"), e.find("i").removeClass().addClass("fa fa-envelope")
            }
        }), !1
    })
